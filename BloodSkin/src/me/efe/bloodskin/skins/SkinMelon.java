...
public class SkinMelon extends Skin {
	
	public SkinMelon() {
		super("melon", true, true, "¸Þ·Ð", new ItemStack(Material.MELON_BLOCK));
	}
	
	@Override
	public void effect(Player p, final LivingEntity entity) {
		ParticleEffect.ITEM_CRACK.display(new ParticleEffect.ItemData(Material.MELON, (byte) 0), 0.1F, 0.1F, 0.1F, 0.1F, 30, getBody(entity), 32);
		
		for (int i = 0; i < new Random().nextInt(2) + 2; i ++) {
			final int id = FakeEntityUtils.generateId();
			
			FakeEntityUtils.spawnItem(getBody(entity), new ItemStack(Material.MELON), id);
